...
test('groupByStructureAndName — 구조+컴포넌트명 둘 다 같아야 한 세트 + 미인식 제외', () => {
  const btn = (id, child) => ({ id, name: 'btn', type: 'FRAME', width: 100, height: 40, paddingTop: 8, layoutMode: 'HORIZONTAL', fillHex: '#111', children: [{ id: id + '-c', name: child, type: 'TEXT' }] });
  ...
  // a,b: 같은 구조·이름(btn). c: 같은 이름이지만 자식 다름(구조 다름) → 별도. x: 미인식 → 제외.
  const groups = groupByStructureAndName([btn('a', 'Label'), btn('b', 'Label'), btn('c', 'Icon'), blob]);
  assert.equal(groups.length, 2);
...
test('deriveVariants — 혼합(무속성 + 속성) 멤버는 빈 변형 이름이 없어야', () => {
  const m = (id, name) => ({ id, name, type: 'FRAME', width: 100, height: 40, fillHex: '#2d7ff9' });
  // nav-button(무속성) + button-primary(Type) → 끝명사 button으로 묶이지만 무속성 멤버는 Variant로 채움
