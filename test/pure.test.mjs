...
/* ================= entitlements.ts (Free/Paid 2티어) ================= */
test('hasEntitlement — 모든 유료 기능은 Paid에서만 해금', () => {
  for (const f of ['tokens', 'semantics', 'components', 'presets']) {
    assert.equal(hasEntitlement('free', f), false);
    assert.equal(hasEntitlement('paid', f), true);
  }
});

test('isPaid — Paid만 true', () => {
  assert.equal(isPaid('paid'), true);
  assert.equal(isPaid('free'), false);
});

test('isTier — free/paid만 유효(구 pro/team 거부)', () => {
  assert.equal(isTier('free'), true);
  assert.equal(isTier('paid'), true);
  assert.equal(isTier('pro'), false); // 2티어 전환으로 폐기
