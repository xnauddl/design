...
test('clusterColorsByDeltaE — 단일 연결 체이닝 방지(군집 폭 ≤ 허용오차)', () => {
  // 그레이 램프는 인접 단계마다 ΔE<8이라 연결요소만 쓰면 전이로 한 덩어리가 된다.
  // (#f8f9fa↔#ced4da 는 ΔE 11.5로 허용오차 밖) → 대표색 반경으로 갈라져야 한다.
  const ramp = ['#f8f9fa', '#e9ecef', '#dee2e6', '#ced4da'].map((hex, i) => ({ name: `gray-${i}`, hex }));
  const clusters = clusterColorsByDeltaE(ramp);

  assert.ok(clusters.length > 1, '체이닝으로 한 군집이 되면 안 됨');
  for (const cl of clusters) {
    for (const m of cl.members) {
      const d = deltaEOK(m.hex, cl.representative.hex);
      assert.ok(d <= DEFAULT_DELTA_E, `${m.name}이 대표에서 ΔE ${d.toFixed(2)} — 허용오차 초과`);
    }
  }
  // 모든 입력색은 정확히 한 번씩 어느 군집엔가 속한다(누락·중복 없음).
