...
test('renameSelection — 구 리네임이 남긴 토큰 베낌 이름(color-121210)은 교체', async () => {
  ...
  // #7b-1: 루트는 항상 보존되므로 서브젝트를 래퍼 안에 넣어 분류 로직을 검증.
  const root = { type: 'FRAME', id: 'root', name: 'Wrapper', children: [frame] };
...
test('renameSelection — swatch 규칙: 색만 채운 빈 프레임 → swatch, 이미지 → image, 빈 → container', async () => {
  ...
  // #7b-1: 루트는 항상 보존 → 서브젝트들을 래퍼 하위에 두어 분류 로직 검증.
  const root = { type: 'FRAME', id: 'root', name: 'Wrapper', children: [swatch, imageFrame, emptyFrame] };
  const { changes } = await renameSelection([root], { apply: true, maxDepth: 3 });
  const after = new Map(changes.map((c) => [c.id, c.after]));
...
test('renameSelection — 색이 있어도 자식이 있으면 스와치가 아니라 컨테이너', async () => {
  ...
  const root = { type: 'FRAME', id: 'root', name: 'Wrapper', children: [card] };
  const { changes } = await renameSelection([root], { apply: true, maxDepth: 3 });
  const after = new Map(changes.map((c) => [c.id, c.after]));
...
test('renameSelection — apply:false면 미리보기만(노드 이름 불변)', async () => {
  ...
  const root = { type: 'FRAME', id: 'root', name: 'Wrapper', children: [node] };
...
test('renameSelection — 버튼 추론: 오토레이아웃+라운드+채움+텍스트 → button', async () => {
  ...
  const root = { type: 'FRAME', id: 'root', name: 'Wrapper', children: [btn] };
  const { changes } = await renameSelection([root], { apply: true, maxDepth: 3 });
  const after = new Map(changes.map((c) => [c.id, c.after]));
...
test('renameSelection — #7b-1: 루트 컨테이너는 기본명이어도 보존', async () => {
  installFigma();
  // 색만 채운 빈 프레임 = 원래라면 swatch. 그러나 선택의 루트이므로 보존.
  const root = { type: 'FRAME', id: 'root', name: 'Frame 1', fills: [{ type: 'SOLID', visible: true }], children: [] };
  const { changes, nodes } = await renameSelection([root], { apply: true, maxDepth: 3 });
  assert.equal(changes.length, 0);
  assert.equal(root.name, 'Frame 1'); // 적용해도 루트는 불변
  assert.equal(nodes.length, 1);
  assert.equal(nodes[0].preserved, 'root');
  assert.equal(nodes[0].changed, false);
});

test('renameSelection — #7b-2: 인스턴스 하위 서브트리는 통째로 스킵', async () => {
  installFigma();
  const innerDefault = { type: 'FRAME', id: 'inner', name: 'Frame 9', children: [] };
  const inst = { type: 'INSTANCE', id: 'inst', name: 'Card', children: [innerDefault] };
  const root = { type: 'FRAME', id: 'root', name: 'Wrapper', children: [inst] };
  const { changes, nodes } = await renameSelection([root], { apply: true, maxDepth: 3 });
  assert.equal(innerDefault.name, 'Frame 9'); // 인스턴스 하위 미방문 → 불변
  assert.equal(changes.find((c) => c.id === 'inner'), undefined);
  assert.equal(nodes.find((n) => n.id === 'inner'), undefined); // 트리에도 없음(스킵)
  assert.equal(nodes.find((n) => n.id === 'inst').preserved, 'instance');
});

test('renameSelection — 미리보기 트리: 전체 서브트리 + parentId/depth/preserved', async () => {
  installFigma();
  const icon = { type: 'VECTOR', id: 'ic', name: 'Vector 1' };
  const txt = { type: 'TEXT', id: 'tx', name: 'Hi', characters: 'Hi' };
  const inner = { type: 'FRAME', id: 'in', name: 'Frame 2', children: [icon, txt] };
  const root = { type: 'FRAME', id: 'root', name: 'Frame 1', children: [inner] };
  ...
  assert.equal(byId.get('root').preserved, 'root');
  assert.equal(byId.get('in').depth, 1);
  assert.equal(byId.get('in').parentId, 'root');
  assert.equal(byId.get('in').changed, true);
  assert.equal(byId.get('in').after, 'container');
  assert.equal(byId.get('ic').parentId, 'in');
  assert.equal(byId.get('ic').depth, 2);
  assert.equal(byId.get('ic').changed, true);
  assert.equal(byId.get('tx').preserved, 'text'); // 텍스트는 맥락으로만(보존)
  assert.equal(byId.get('tx').changed, false);
});
