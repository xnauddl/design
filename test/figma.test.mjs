...
/* ================= variables.ts ================= */
test('createTokens — Global 리터럴만 생성(Semantic 미러 없음) + scopes/hidden + px 스냅샷', async () => {
  ...
  // 패턴 C: Global 원시값만. Semantic은 시맨틱 매핑(역할 별칭) 단계에서. 토큰 3개 → Global 3.
  assert.deepEqual(summary, { created: 3, updated: 0, globals: 3 });
  ...
  // Semantic 컬렉션엔 원시명 미러가 생기지 않는다(정의: Semantic=역할명만)
...
test('createTokens — 재실행 멱등(upsert): 두 번째는 모두 updated', async () => {
  ...
  assert.equal(first.created, 2); // Global만(토큰 2개)
  assert.deepEqual(
...
/* ================= bind.ts ================= */
// 패턴 C: createTokens는 Global만 만든다. 바인딩 대상(Semantic/Component)은 역할 별칭으로 깔아준다.
async function seedBindable(tokens, roleMap, base = 16) {
  await createTokens(tokens, base);
  await createSemanticAliases(roleMap);
}

test('bindSelection — 색/크기 바인딩, 미매칭 skip, 오토레이아웃 아님 플래그', async () => {
  const figma = installFigma();
  // Semantic 역할 별칭 시드(→Global 리터럴): primary→#0066ff, size/lg→200
  await seedBindable(
    [
      { name: 'color/0066ff', category: 'color', sources: ['fill'], value: '#0066ff' },
      { name: 'size/200', category: 'size', sources: ['size'], value: 200 },
    ],
    { primary: 'color/0066ff', 'size/lg': 'size/200' },
  );
  ...
  // width가 Semantic 역할 변수(size/lg)로 바인딩됨
  const sSize = findVar(figma, 'Semantic', 'size/lg');
...
test('bindSelection — 허용오차 내 동률은 가장 가까운 값으로 바인딩', async () => {
  ...
  const s12 = findVar(figma, 'Semantic', 'size/md'); // 12 → 역할 size/md
...
test('bindSelection — 여백(padding/gap)은 GAP 변수에만 — size/line-height/letter-spacing 오매칭 방지', async () => {
  ...
  const gap = findVar(figma, 'Global', 'spacing/24'); // 원시값은 Global, 바인딩은 Global 폴백
...
test('bindSelection — 선 두께(strokeWeight)는 STROKE_FLOAT 변수에 바인딩(여백 변수로 안 샘)', async () => {
  ...
  const sw = findVar(figma, 'Global', 'stroke-width/2'); // 원시값은 Global, 바인딩은 Global 폴백
...
test('bindSelection — 색상도 용도 스코프로 분리(stroke 전용 색은 fill에 안 붙음)', async () => {
  ...
  const fillVar = findVar(figma, 'Global', 'color/aa0000'); // 원시값은 Global, 바인딩은 Global 폴백
  const strokeVar = findVar(figma, 'Global', 'color/0000aa');
...
test('bindSelection — 레이어 불투명도(opacity)는 OPACITY 변수에 정밀 바인딩', async () => {
  ...
  const op = findVar(figma, 'Global', 'opacity/0_5'); // 원시값은 Global, 바인딩은 Global 폴백
...
test('bindSelection — fontFamily(STRING)는 FONT_FAMILY 변수에 정확 일치 바인딩', async () => {
  ...
  const fam = findVar(figma, 'Global', 'font-family/Inter'); // 원시값은 Global, 바인딩은 Global 폴백
...
test('bindSelection — 사용량 한도(maxNodes) 초과 시 부분 적용 + limited', async () => {
  installFigma();
  await seedBindable([{ name: 'color/0066ff', category: 'color', sources: ['fill'], value: '#0066ff' }], { primary: 'color/0066ff' });
  const mk = (id) => ({
    type: 'FRAME',
    id,
    name: id,
...
test('bindSelection — dry-run 후보(#6) + 트리 노드(#13): 영향+조상, 필드/변수/인덱스', async () => {
  ...
  assert.equal(fillC.variableName, 'primary'); // 역할 별칭으로 바인딩
...
test('previewCreateTokens — 변수 생성 없이 생성/갱신 예정 집계', async () => {
  ...
  // 토큰 2개 → Global 2(Semantic 미러 없음)
  assert.equal(p.globals, 2);
  assert.equal(p.semantics, undefined);
...
test('prunePaletteColors(#3) — 재생성 hue 패밀리 안에서만 정리(다른 패밀리·추출 hex 보존)', async () => {
  ...
  assert.equal(removed, 1); // blue/700 Global(원시명 Semantic 미러는 더 이상 없음)
