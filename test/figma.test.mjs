...
test('bindSelection — 여백(padding/gap)은 GAP 변수에만 — line-height/letter-spacing 오매칭 방지', async () => {
  const figma = installFigma();
  // 값이 모두 24로 같은 세 변수: 간격(GAP) · 행간(LINE_HEIGHT) · 자간(LETTER_SPACING)
  await createTokens(
    [
  ...
  // itemSpacing·paddingLeft는 GAP 변수로만 연결(행간/자간으로 새지 않음)
