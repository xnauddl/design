...
test('alignFrames — 동일 구조 묶고 다른 구조 제외 + 가변 위치 산출', () => {
  ...
  assert.equal(r.imageVarying.length, 0); // 이미지 fill 없음(INSTANCE) → imageVarying 없음
});
...
test('alignFrames — 가변 이미지 fill은 imageVarying(imageHash 비교, v2)', () => {
  const withImg = (id, t, hash) => ({
    id, name: 'Card', type: 'FRAME',
    children: [
      { id: `${id}:pic`, name: 'Photo', type: 'RECTANGLE', hasImageFill: true, imageHash: hash },
      { id: `${id}:t`, name: 'Title', type: 'TEXT', characters: t },
    ],
  });
  // 이미지가 다르면(imageHash 상이) imageVarying — 인스턴스 fill 오버라이드 대상
  const vary = alignFrames([withImg('1', 'A', 'h1'), withImg('2', 'A', 'h2')]);
  assert.deepEqual(vary.imageVarying, ['Photo']);
  assert.deepEqual(vary.varying.map((v) => v.path), []); // 텍스트 동일 → 속성 없음
  // 이미지가 같으면(동일 imageHash) 비가변, 텍스트만 가변
  const same = alignFrames([withImg('1', 'A', 'h1'), withImg('2', 'B', 'h1')]);
  assert.deepEqual(same.imageVarying, []);
  assert.deepEqual(same.varying.map((v) => v.path), ['Title']);
});
