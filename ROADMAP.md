...
- [x] **유료 게이팅 노출** — 게이팅 중인 유료 카드(컴포넌트·텍스트 스타일=Pro, 프리셋=Team) 🔒 배지 일관화(`updateGates`); 토큰/시맨틱 등 2티어 신규 Paid 경계는 `(PR #30)`
...
> **플러그인 코드 전환 완료**(`entitlements` 2티어 · 단일 `requirePaid` · UI 배지/구독링크 · `__DEV__` 백도어 차단) `(PR #55)`.
> 검증 Worker·키쌍·URL 등 **배포 인프라**는 PR #30(`workers/verify`)에서 별도 관리 — 배포만 남음.
> 후속: README 인라인 티어 라벨(Pro/Team→Free/Paid) 정리.
