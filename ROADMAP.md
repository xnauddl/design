...
- [x] **닮은 프레임 컴포넌트화** — 구조 동일·내용만 다른 프레임 N개 → 마스터 1개 컴포넌트화 + 가변 텍스트/인스턴스를 컴포넌트 속성으로 노출, 나머지는 오버라이드 인스턴스로 교체. v2: 가변 이미지 fill은 인스턴스 fill 오버라이드(`similar.ts`·`similarApply.ts`, 순수+목 테스트). `(PR #55)`
...
- [x] **유료 게이팅 노출** — 모든 유료 카드(토큰·시맨틱·컴포넌트·텍스트 스타일·프리셋)에 🔒 Paid 배지 일관화(`updateGates`) `(PR #55)`
- [x] **단계별 의존관계 시각화** — 시작 탭 ‘진행 안내’ 파이프라인(`pipeline.ts`): 토큰 생성→시맨틱→바인딩 단계 상태(완료/준비됨/전제 미충족) + 클릭 이동(`PREREQ_STATE` 연동) `(PR #41)`
...
> **플러그인 코드 + 검증 인프라 완료** `(PR #55)` — `entitlements` 2티어 · 단일 `requirePaid` ·
> UI 배지/구독링크 · `__DEV__` 백도어 차단 · 검증 Worker(`workers/verify`, 목 테스트 15건) ·
> 키쌍 생성기(`scripts/gen-license-keys.mjs`). PR #30(구 main 기반 2티어)은 **#55로 대체**.
> 남은 건 **실제 배포**(Worker 배포 · URL/키 교체 · LS 스토어 설정)뿐.
...
### 보안 / 관리
- [x] **개발용 티어 토글을 `__DEV__` 빌드 플래그로 dev 빌드 전용화**(배포 빌드 백도어 차단) `(PR #55)`
- [x] **오너 관리자 키**(Worker allowlist) — 결제 없이 paid 토큰(스모크 테스트) `(PR #55)`
...
- [x] **2티어 전환 + 기능 게이팅 + 검증 Worker(C) 작성** `(PR #55 — PR #30 대체)`
...
- [x] 결제 제공자 선택(**LemonSqueezy**) · 검증 서버 코드(**Cloudflare Worker**) 작성 — *배포만 남음* `(PR #55)`
- [x] 가격 확정 — **연 $39**
