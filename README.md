...
- UI: **만들기** — ‘선택에서 스캔’ → 표(이름만 편집·스캔값 읽기 전용, 신규=앰버/등록됨=파랑, 행간·자간은 `24px`/`150%`·`0.32px`/`-2%`처럼 단위까지 표시) → ‘텍스트 스타일 등록’(+화면 적용 체크). **적용** — ‘텍스트 스타일 적용’(변수 바인딩과 별 카드). 순수 로직은 `node --test`, figma 호출은 `variables.ts`. **Paid 게이팅**(스캔은 무게이팅 미리보기).
...
- ✅ **국제화(i18n)**: UI 문자열을 `i18n.ts`의 `STRINGS` 단일 소스로 모으고 `t(key, vars)`로 조회(`{var}` 보간, 누락 키는 폴백). 상태/피드백·정적 라벨·마법사 문구·툴팁(`data-i18n-title`)·자리표시자(`data-i18n-placeholder`)·선택지까지 외부화. `verify-i18n`이 키↔폴백 드리프트를 검사.
