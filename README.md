...
- 위변조 방지: 검증은 서버에서만, 응답 **서명(JWT 등)** 은 M2.1. `manifest.json`의 `networkAccess.allowedDomains`에 검증 도메인 명시(배포 시 실제 도메인으로 교체).
- 순수 로직(`license.ts`: 캐시 평가·응답 파싱)은 `node --test`로 검증, 실제 `fetch`·`clientStorage`만 `code.ts`.
...
- **M2 ✅ (구현됨, 서버 미배포)**: 라이선스 키 입력·검증(`license.ts` + `code.ts` fetch) + `clientStorage` 캐시·오프라인 grace. `VERIFY_URL`·`allowedDomains`는 자리표시 → 실제 검증 서버/결제 제공자 연동 시 교체. 응답 서명(JWT)은 M2.1.
- **M3**(다음): Team(시트·공유 프리셋·이력), 가격·프로모션 확정.
