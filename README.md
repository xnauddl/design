...
### 라이선스 아키텍처 (외부 키)
- 웹 결제(결제 제공자: Gumroad/LemonSqueezy/Paddle 또는 자체 서버) → **라이선스 키 발급** → 플러그인 설정에 키 입력 → `code.ts`가 **검증 서버**에 요청 → 엔타이틀먼트 응답.
- 검증 서버 책임: 키 진위·구독 상태·만료·시트 확인. **디자인 데이터 미전송**(키 + plugin/user 식별자만).
- `figma.clientStorage` 캐시: `{ entitlements, expiresAt, lastVerified, graceUntil }`. 온라인 시 재검증, **오프라인 유예(grace, 예: 7–14일)**.
- 위변조 방지: 검증은 서버에서만, 응답 **서명(JWT 등)** 권장. `manifest.json`의 `networkAccess.allowedDomains`(현재 `["none"]`)에 **검증 도메인 추가** 필요.

### 엔타이틀먼트 모델 (코드 구조 · 구현은 추후 PR)
- 기능 플래그 집합 + 단일 게이트 `hasEntitlement(feature)`. `src/code.ts`의 메시지 switch 유료 case 진입부(또는 핸들러 내부)에서 검사.
- **사용량 한도 집행**: Free일 때 핸들러가 처리량을 한도까지 잘라 적용하고 초과분은 `PREMIUM_REQUIRED`로 안내(결과에 `limited` 플래그). 한도 상수는 한 곳에서 관리.
- 추가 예정 메시지(`src/shared/messages.ts`): `UiToCode`에 `SET_LICENSE_KEY` · `GET_LICENSE`; `CodeToUi`에 `LICENSE_STATUS` · `PREMIUM_REQUIRED`(초과/유료 사유 포함).
...
### 단계별 출시
- **M0**(현재): 전 기능 무료.
- **M1**: 엔타이틀먼트 스캐폴드 + **사용량 한도 집행**(개발용 가짜 라이선스 토글), 결제 없음.
- **M2**: 외부 결제 + 검증 서버 + 키 입력/검증 + `clientStorage` 캐시·grace.
- **M3**: Team(시트·공유 프리셋·이력), 가격·프로모션 확정.
