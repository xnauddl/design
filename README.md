...
- **배포 인프라(`PR #55`)**: Cloudflare Worker(`workers/verify`, LS `activate`/`validate`→ES256 JWT 발급)·키쌍 생성기(`scripts/gen-license-keys.mjs`)는 작성·목 테스트 완료. 남은 건 배포: `licenseConfig.ts`의 `VERIFY_URL`·공개키·`PURCHASE_URL`·`PORTAL_URL`·`manifest.json allowedDomains` 실제 값 교체(가이드 `workers/verify/README.md`).
