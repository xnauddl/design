...
- **설정은 관리에만 ✅ (v2 #19)**: 기준 크기(base)·리네임 맥락 깊이는 **관리 탭 「플러그인 설정」 한 곳**에서만 바꾸고 `clientStorage`(`dsl.settings`)에 자동 저장된다. **바인딩 허용오차는 0.5px 고정**(칩·입력 제거 — 0은 반올림 때문에 안 붙고 1↑는 과매칭), **여백·크기 격자는 8px 고정**으로 미리보기 때 자동 스냅한다. 두 고정값은 설정 카드에 문구로 표시.
