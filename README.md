...
패밀리 기준으로 **색상** 추천 매핑을 채워주며, `역할 = Global변수이름` 형식으로 편집 후 적용합니다. 적용은
`createSemanticAliases`가 원시 스코프를 상속한 별칭 변수를 upsert(멱등)로 생성합니다.

**비색상 시맨틱(간격·반경)**: 토큰 추출/팔레트 생성 시 `suggestNonColorSemanticMap`이 기존 숫자 토큰을
기준으로 **티셔츠 스케일**을 추천해 같은 매핑에 병합합니다(`space/md → spacing/16`, `radius/lg → radius/16`,
`radius/full`=최대 반경). 비색상 별칭은 원시 스코프(GAP·CORNER_RADIUS)를 그대로 상속하므로 추가 설정이
필요 없습니다. 추천은 **기존 입력을 덮지 않고 병합**(사용자 편집 우선). 추천 산출은 순수 함수라 `node --test`로 검증.
> 타이포 축별 시맨틱(`font-size/body`)과 시맨틱 텍스트 스타일(`heading`·`body`)은 후속 단계에서 추가됩니다.
