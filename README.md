...
src/
  ...
  lib/
    ...
    color.ts    색공간 변환(sRGB↔OKLab↔OKLCH) — 순수
...
- **스캔**(`SCAN_TEXT_STYLES`): 선택 트리의 **보이는** TEXT 노드에서 `{fontSize, lineHeight(px), letterSpacing, family, style}` 시그니처를 수집(`scanTextStyles`). 숨김(`visible=false`)·부분 서식(mixed)은 스킵(+경고). PERCENT 자간·행간은 px 환산 — 다만 **원본 %는 `lineHeightPercent` / `letterSpacingPercent`로 함께 보존**한다(시그니처·매칭 기준은 계속 px).
...
- **행간·자간 % 보존**: 원본이 %인 행간/자간은 스타일에 **`PERCENT`로 등록**하고 그 스타일의 **해당 변수 바인딩만 생략**한다(다른 축 바인딩은 유지). Figma는 행간·자간에 변수를 바인딩하면 단위를 `PIXELS`로 강제하고, 변수는 단위 없는 FLOAT이라 `%`와 바인딩은 동시에 가질 수 없다. 이미 %로 만들어 둔 기존 스타일을 rename할 때도 같은 이유로 바인딩하지 않는다. 변수는 그대로 만들되 **값은 px 스냅샷 + 원본 단위는 `description`("150%"·"-2%")** — 내보내기가 description을 우선한다(#16 규칙). 같은 px 이름에 원본이 갈리면 px로 기록하고 알림을 남긴다. 의도된 미바인딩은 경고(`missing`)가 아니라 `notes`로 보고.
- **적용**: (a) 등록 시 옵션(‘등록 시 화면에 적용’) — 전체 시그니처 일치 노드에 연결. (b) **‘기존만 연결’**(`APPLY_TEXT_STYLES`) — 생성 없이 시그니처 매칭으로 Local Text Style만 연결(미등록·모호는 보고).
- UI: **만들기 · 타이포** 한 카드에서 끝난다 — 툴바 ‘스캔 · 행 추가 / 등록 · 기존만 연결’ → 결과는 표가 아니라 **리스트 + ×N 배지**(#17: 이름만 편집·스캔값 읽기 전용, 신규=앰버/등록됨=파랑, 요약에 `Inter · 24 / 150% / -2% · SemiBold`처럼 단위까지). 순수 로직은 `node --test`, figma 호출은 `variables.ts`. **Paid 게이팅**(스캔은 무게이팅 미리보기).
...
- UI 「변수 → 코드」 카드(관리 탭)에서 형식·폰트단위 선택 → **「내보내기」 한 번에 `tokens.json`/`tokens.css` 파일 저장**. 패널에 코드 미리보기는 두지 않는다(#8) — 좁은 패널의 textarea로 코드를 읽는 사람은 없고, 결과물은 파일이다. 내보낼 변수가 없으면 빈 껍데기(`{}`·`:root {}`)를 저장하는 대신 안내한다(`exportHasTokens`). 게이팅: **Free**(리드젠).
...
**툴바 배치**: 구조 카드 툴바는 와이어대로 **스캔→등록 4개**(후보 스캔·닮은 스캔 / 컴포넌트 등록·컴포넌트화)만 둡니다. 「베리언트 분류」·「누락 조합 생성」은 *이미 만든* 컴포넌트를 손보는 뒷일이라 상시 노출하지 않고, 대상이 실제로 선택됐을 때만 결과 목록 아래 줄로 나타납니다 — 판정 기준은 두 핸들러가 잡는 것과 같습니다(세트에 안 속한 낱개 `COMPONENT` = 분류 / `COMPONENT_SET` = 누락 조합). 수는 `SELECTION_STATE.comps`·`sets`로 옵니다.
...
- **마법사 첫 화면 ✅ (v2)**: 온보딩 카드를 없애고 **첫 실행 배너** 한 줄로 흡수한다(‘다시 보지 않기’는 파일에 기억 — `dsl.settings.hideOnboard`). 실행 중 단계는 **미니 파이프 칩**(실제로 도는 단계만), 의존 파이프라인은 **‘진행’** 3행 스트립.
- **텍스트 스타일 목록 ✅ (v2 #17)**: 표 대신 **리스트 + ×N**. 스캔 값은 편집 대상이 아니므로 요약 한 줄로 접고, 이름만 고친다. `×N` 배지는 그 스타일을 쓰는 글자 수이며 누르면 캔버스에서 선택한다(1개면 그 글자로 이동). 좁은 패널에서 열 폭을 다투지 않는다.
- **결과 목록 ✅ (v2 #18)**: 스캔·미리보기 결과는 **얇은 카드(`r-card`) 스택** 한 문법으로 통일한다 — 열은 `[체크 | 제목·요약 | 메타]` 셋이고, 트리는 **그룹 머리 + 자식 카드**로 그린다(토큰·색·바인딩·리네임·컴포넌트 후보·닮은 프레임·베리언트 리포트 전부). 목록마다 두던 `max-height` 스크롤 박스는 없앴다 — **패널 body 하나가 스크롤**한다(그래서 행 높이 정수배 스냅·‘모두 펼치기’ 장치도 함께 사라졌다).
- **단계 레일 ✅ (v2 #5)**: 만들기는 **색 → 토큰 → 테마 → 타이포** 4단계, 적용은 **바인딩 → 리네임 → 구조** 3단계로 나뉘고 **한 화면에 한 단계**만 보인다(카드는 마크업의 `data-make-step`/`data-apply-step`으로 묶임). 전제 안내의 ‘토큰 생성으로 →’ 같은 바로가기는 탭과 단계를 함께 맞춘다.
- **설정은 관리에만 ✅ (v2 #19)**: 기준 크기(base)·리네임 맥락 깊이는 **관리 탭 「플러그인 설정」 한 곳**에서만 바꾸고 `clientStorage`(`dsl.settings`)에 자동 저장된다. **바인딩 허용오차는 0.5px 고정**(칩·입력 제거 — 0은 반올림 때문에 안 붙고 1↑는 과매칭), **여백·크기 격자는 8px 고정**으로 미리보기 때 자동 스냅한다. 두 고정값은 설정 카드에 **비활성 필드 + 「고정」**으로 표시한다(회색 문장으로 적으면 읽히지 않는다). 자세한 규칙은 필드 툴팁.
...
- ❌ **명도 대비 점검 — 제거(비목표)**: 이 플러그인의 일이 아니라고 판단해 카드·마법사 단계·메시지·`lib/contrast.ts`를 전부 걷어냈다. 접근성 검사는 전용 플러그인(Stark 등)이 더 잘한다. 함께 있던 보정 기능은 토큰을 거치지 않고 노드에 날 hex를 박아 같은 색을 쓰던 형제들과 갈라놓았고, 변수가 바인딩된 채움에서는 변수 값이 이겨 실제로 아무것도 바꾸지 못했다 — 디자인 시스템 도구가 할 일과 정반대였다.
