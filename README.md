...
- 순수 로직(`license.ts` 캐시 평가, `licenseToken.ts` 디코드·클레임)은 `node --test`로 검증, 실제 `fetch`·`clientStorage`·`crypto.subtle`만 `code.ts`(서명 검증 원시연산은 주입).
- 메모: Figma 메인 샌드박스에 WebCrypto가 없을 수 있어, 운영 서명 검증은 **UI 아이프레임으로 이동(M2.2)** 예정. `VERIFY_URL`·공개키·`allowedDomains`는 자리표시(배포 시 교체).
...
- **M2.1 ✅ (구현됨)**: 서명 토큰(JWT) 검증 코어(`licenseToken.ts`) — 서명+클레임 검사, `alg=none` 거부. WebCrypto 검증은 주입식(`code.ts`).
- **M2.2**(다음): 서명 검증을 UI 아이프레임(WebCrypto 가용)으로 이동, 실제 공개키/서버 연동.
