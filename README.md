...
- UI: **‘선택에서 스캔’** → 표(이름만 편집·스캔값 읽기 전용, 신규=앰버/등록됨=파랑, 행간·자간은 `24px`/`150%`·`0.32px`/`-2%`처럼 단위까지 표시) → **‘텍스트 스타일 등록’** (+화면 적용 체크) · **‘기존 스타일 적용만’**. 순수 로직은 `node --test`, figma 호출은 `variables.ts`. **Paid 게이팅**(스캔은 무게이팅 미리보기).
