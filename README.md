...
- **컴포넌트 등록(registration)**
  ...
  - **엄격 필터**: 이름에 알려진 **컴포넌트 명사**(`button`/`card`/`chip`/`badge`/`tab`… + 약어 `btn`)가 있는 `FRAME`/`GROUP`만 추린다. 이름 없는 프레임(`Frame 12`)·`INSTANCE`·잠금·`TEXT`는 제외.
  - **그룹화 = 구조 + 컴포넌트명** 둘 다 같아야 한 세트(잘못 묶임 방지). 세트 이름은 멤버 공통 베이스를 **PascalCase**로(약어 펼침 `btn`→`Button`). 단독 컴포넌트는 원본 이름 유지.
