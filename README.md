...
오픈 코어 + **프리미엄(Freemium)** 모델로, 무료 핵심 기능은 그대로 두고 팀·고급 기능을 유료로 전환할 수 있게 설계한다. 무료 사용자의 기존 동작은 **절대 막지 않으며**(파괴적 게이팅 금지), 유료 기능은 라이선스가 없으면 **미리보기 + 잠금 안내**로만 노출한다.

- **티어(안)**
  - **Free** — 추출 · 토큰 생성(3계층) · 바인딩 · 리네임. 개인/소규모 사용.
  - **Pro** — 컴포넌트 등록·베리언트 분류(Phase 3), 라이브러리 발행, 멀티모드/테마(라이트·다크) 관리, 누락 조합 리포트·자동 생성, AI 네이밍 제안.
  - **Team** — 팀 공유 프리셋/네이밍 규칙, 대규모 일괄 처리, 변경 이력, 우선 지원.
- **과금 · 엔타이틀먼트**
  - 결제는 **외부(웹)** 에서 처리, 플러그인은 **라이선스 상태만 검증**(라이선스 키 또는 Figma 결제 연동). 검증 API 도메인은 `manifest.json`의 `networkAccess.allowedDomains`에 명시.
  - 라이선스 상태는 `figma.clientStorage`에 캐시 + **오프라인 유예(grace) 기간**. 키 진위 검증은 서버에서만.
  - 가격·티어 구성은 **추후 확정**(자리표시).
- **게이팅 원칙**
  - 무료 기능은 라이선스와 무관하게 항상 동작. 유료 기능은 실행 전 단계까지 **미리보기 허용**, 적용(쓰기)만 잠금.
  - 코드 구조상 **기능 플래그(entitlement)** 로 분기하여 무료/유료 경로를 한 곳에서 관리.
- **프라이버시**
  - 토큰·디자인 데이터는 **로컬 처리** 원칙 유지. 외부 전송은 **라이선스 검증 요청에 한정**하고 디자인 콘텐츠는 보내지 않는다.

> 비고: 이 섹션은 **사업/구현 방향의 자리표시(플레이스홀더)** 이며, 실제 결제·라이선스 연동과 기능 게이팅은 별도 설계/PR에서 진행한다.
