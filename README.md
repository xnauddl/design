...
**수동 입력.** `2.5 · 시맨틱 매핑` 카드의 입력칸은 자유 편집이며, 자동 추천은 이 칸을 미리 채워줄 뿐입니다.
추천 줄을 지우거나 직접 줄을 더해 자동·수동을 한 칸에서 섞을 수 있습니다. 한 줄에 하나씩
`역할 = Global변수이름` 형식으로 적습니다. 파싱(`textToSemanticMap`)은 `=` 기준으로 나누고 앞뒤
공백을 제거하며, `=` 없는 줄은 무시합니다(빈 줄 안전). 같은 역할명을 두 번 쓰면 나중 줄이 이깁니다.
**오른쪽은 반드시 이미 존재하는 Global 변수 이름**이어야 하고(없으면 `missing`으로 보고·스킵 —
`2 · 토큰 생성` 선행 필요), **왼쪽 역할의 머리말(슬래시 앞)이 적용 스코프를 결정**합니다
(`text/*`→텍스트, `border/*`→선, `surface/*`·`background/*`→프레임 채움, 그 외는 원시 스코프 상속 —
`scopeForSemanticRole`). 멱등이라 재실행은 안전합니다.
