...
**Phase 4 — 누락 조합 자동 생성**: 선택한 베리언트 세트의 **빠진 속성 조합**(`missingVariants` 순수 계산)을 기존 변형을 클론해 `prop=value`로 이름 지정하여 생성(`GENERATE_MISSING_VARIANTS`, Pro). 라이브러리 발행은 Plugin API 미지원이라 수동, 컴포넌트 속성 노출은 Phase 4.1.
