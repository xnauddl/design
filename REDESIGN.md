...
| 탭 id | 라벨 | 카드 | 마크업 |
|---|---|---|---|
| `tokens` | 만들기 | 팔레트 생성·추출·토큰 생성·시맨틱 매핑·텍스트 스타일 등록 | ui.html |
| `apply` | 적용 | 변수 바인딩·텍스트 스타일 적용·리네임·명도 대비·컴포넌트/베리언트 | ui.html |
...
- **✅ 결정**: **4탭 — `시작`(마법사) · `만들기` · `적용` · `관리`**, **색은 만들기 안**.
  - `만들기`: 팔레트 생성/추출 → **색 편집표** → 토큰 생성 → 시맨틱 매핑 → 텍스트 스타일 **등록** (한 파이프라인)
  - `적용`: 변수 바인딩 · **텍스트 스타일 적용** · 리네임 · 대비 · 컴포넌트 / `관리`: 내보내기·요금제·프리셋(이력 제거)
