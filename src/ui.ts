...
function showTab(name: (typeof TABS)[number]): void {
  ...
  // 비활성 탭은 display:none이라 그 안의 목록은 높이 0으로 측정되고 스냅이 조용히 bail한다
  // (접힌 카드와 같은 결함 — applyCardChrome 참고). 결과가 다른 탭에 있는 동안 도착하는
  // 경우가 실제로 있다: 마법사의 대비 점검이 #contrastList(적용 탭)를 채운다. 탭을 여는
  // 지금 다시 재지 않으면 반쪽 행과 빈 개수 줄이 그대로 남는다.
  layoutAllLists();
