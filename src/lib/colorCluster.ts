...
export function clusterColorsByDeltaE(
  ...
  const pickRepresentative = (idxs: readonly number[]): number => {
    let best = idxs[0];
    for (const i of idxs) {
      const p = pts[i];
      const q = pts[best];
  ...
  const dist = (i: number, j: number): number =>
    Math.hypot(pts[i].L - pts[j].L, pts[i].a - pts[j].a, pts[i].b - pts[j].b) * 100;

  const out: ColorCluster[] = [];
  for (const r of order) {
    // 연결요소는 단일 연결(single-link)이라 A~B~C 전이로 군집 폭이 허용오차를 넘을 수 있다
    // (예: 그레이 램프는 인접 단계마다 ΔE<8이어도 양끝은 훨씬 멀다).
    // 대표색 반경으로 한 번 더 거른다 — 대표에서 ΔE>tolerance인 색은 떼어내
    // 남은 것들끼리 같은 규칙으로 다시 대표를 뽑는다(대표색 기준 최대 반경 보장).
    let rest = groups.get(r) as number[];
    while (rest.length) {
